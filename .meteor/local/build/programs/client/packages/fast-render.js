...
var FastRender, __init_fast_render, Cookies, DeepExtend, deepExtend, Log;
...
__init_fast_render = function(ejsonJson) {                                                                       // 1
  var initData = EJSON.fromJSONValue(ejsonJson);                                                                 // 2
                                                                                                                 // 3
...
var insideIronRouter = false;                                                                                    // 5
var RouteController = FastRender.RouteController = Package['iron-router'].RouteController;                       // 6
                                                                                                                 // 7
var originalRun = RouteController.prototype.run;                                                                 // 8
RouteController.prototype.run = function() {                                                                     // 9
  if(FastRender.enabled) {                                                                                       // 10
    insideIronRouter = true;                                                                                     // 11
    originalRun.call(this);                                                                                      // 12
    insideIronRouter = false;                                                                                    // 13
  } else {                                                                                                       // 14
    originalRun.call(this);                                                                                      // 15
  }                                                                                                              // 16
};                                                                                                               // 17
                                                                                                                 // 18
var originalSubscribe = Meteor.subscribe;                                                                        // 19
Meteor.subscribe = function(subscription) {                                                                      // 20
  var condition =                                                                                                // 21
    FastRender.enabled &&                                                                                        // 22
    //need to inside the ironRouter                                                                              // 23
    insideIronRouter &&                                                                                          // 24
    //path loaded from the server and the local Router path should be the same                                   // 25
    //We can't simply use Router.current().path, it will give some weird deps behaviour                          // 26
    //which will result subscriptions stop everytime even they are not meant to                                  // 27
    getPath() == __fast_render_config.serverRoutePath &&                                                         // 28
    //fast render have been registered to handle this subscription                                               // 29
    __fast_render_config.subscriptions[subscription] &&                                                          // 30
    //subscription not yet actually loaded (this may call multiple times)                                        // 31
    !__fast_render_config.loadedSubscriptions[subscription]                                                      // 32
                                                                                                                 // 33
  if(condition) {                                                                                                // 34
    Log('APPLY_IR_SUB_CORRECTIONS', subscription);                                                               // 35
    originalSubscribe.apply(this, arguments);                                                                    // 36
                                                                                                                 // 37
    //ironRouter call .ready() and and if it's true he think subscription is completed                           // 38
    return {                                                                                                     // 39
      ready: function() {                                                                                        // 40
        return true;                                                                                             // 41
      }                                                                                                          // 42
    ...
    return originalSubscribe.apply(this, arguments);                                                             // 45
  }                                                                                                              // 46
};                                                                                                               // 47
                                                                                                                 // 48
function getPath() {                                                                                             // 49
  var url = document.createElement('a');                                                                         // 50
  url.href = location.href;                                                                                      // 51
  return url.pathname;                                                                                           // 52
}                                                                                                                // 53
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
if(typeof Meteor.user == 'function') {                                                                           // 1
  Deps.autorun(function() {                                                                                      // 2
    ...
    if(user) {                                                                                                   // 4
      var loginToken = Meteor._localStorage.getItem('Meteor.loginToken');                                        // 5
      var loginTokenExpires = Meteor._localStorage.getItem('Meteor.loginTokenExpires');                          // 6
                                                                                                                 // 7
      Cookies.set('meteor_login_token', loginToken, {                                                            // 8
        path: '/',                                                                                               // 9
        expires: loginTokenExpires                                                                               // 10
      });                                                                                                        // 11
    } else {                                                                                                     // 12
      Cookies.expire('meteor_login_token');                                                                      // 13
    }                                                                                                            // 14
  });                                                                                                            // 15
} else {                                                                                                         // 16
  //make sure cookie is deleted (if previously setted)                                                           // 17
...
//# sourceMappingURL=ab3a941073d15aa885c4810e191110eef1027761.map
