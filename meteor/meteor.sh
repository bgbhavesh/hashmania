...
APP_HOST=http://youtap.youiest.com
...
APP_NAME=Tapmate
ROOT_URL=http://youtap.youiest.com
PORT=80
APP_DIR=../
#APP_DIR=/media/nicolsondsouza/Last/Nicolson/ubuntu/meteor/blog
...
echo forever stop bundle/main.js
rm -rf bundle
echo rm -rf bundle
tar xfz /tmp/bundle.tgz -C $APP_DIR
echo tar xfz /tmp/bundle.tgz -C $APP_DIR
rm /tmp/bundle.tgz
echo rm /tmp/bundle.tgz
pushd bundle/programs/server/node_modules
echo pushd bundle/programs/server/node_modules
rm -rf fibers
echo rm -rf fibers
...
echo chown -R www-data:www-data bundle
...
echo forever start bundle/main.js
popd
echo popd
ENDSSH
